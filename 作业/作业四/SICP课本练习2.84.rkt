...
(define (install-raise-package) ;install raise functions 
  ...
  (define (raise-rational r)
    (make-real (/ (cadr r) (cddr r))))
  (define (raise-real r)
    (make-complex-from-real-imag (contents r) 0))
  (put 'raise 'integer
       (lambda (x) (raise-integer x)))
  (put 'raise 'rational
  ...
  (void))
(define (raise x)
  (apply (get 'raise (type-tag x)) (list x)))

(define (lower? type1 type2)
  (cond ((eq? type1 'integer) #t)
        ((eq? type1 'complex) #f)
        ((eq? type1 'rational)
         (cond ((eq? type2 'integer) #f)
               (else #t)))
        (else (cond ((eq? type2 'complex) #t)
                    (else #f)))))
...
(define (display-obj x)
  (let ((type (type-tag x)))
    (cond ((eq? type 'complex)
           (displayln (list type (caddr x) (cdddr x))))
          ((eq? type 'rational)
           (displayln (list type (cadr x) (cddr x))))
          (else
           (displayln (list type (cdr x)))))))
