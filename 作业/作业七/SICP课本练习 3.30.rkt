...
(define (ripple-carry-adder wire-lstA wire-lstB wire-lstS wireCN)
  (define (ripple-carry-adder-helper a1 b1 s1 c-in)
    (if (null? a1)
        (or-gate c-in c-in wireCN)
        (let ((tmp-c-out (make-wire)))
          (full-adder
           (car a1) (car b1) c-in (car s1) tmp-c-out)
          (ripple-carry-adder-helper
           (cdr a1) (cdr b1) (cdr s1) tmp-c-out))))
  (let ((c-in (make-wire)))
    (set-signal! c-in 0)
    (ripple-carry-adder-helper wire-lstA wire-lstB wire-lstS c-in)))
